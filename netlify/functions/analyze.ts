...
// V2 Default Export (Modern Netlify syntax)
export default async (req: Request): Promise<Response> => {
  ...
  console.log("DEBUG ENV KEYS:", Object.keys(process.env).filter(k => k.includes("API") || k.includes("KEY")));
  console.log("GEMINI_API_KEY len:", process.env.GEMINI_API_KEY?.length || 0, "starts:", process.env.GEMINI_API_KEY?.substring(0, 6));
  console.log("LOVABLE_API_KEY len:", process.env.LOVABLE_API_KEY?.length || 0, "starts:", process.env.LOVABLE_API_KEY?.substring(0, 6));
  ...
  if (!apiKey) {
    return json({
      ok: false,
      code: "missing_key",
      error: "Configure GEMINI_API_KEY nas variáveis de ambiente do Netlify.",
    });
  }
...
// V1 Named Export (Legacy/Standard Netlify handler)
export const handler = async (event: any, context: any) => {
  if (event.httpMethod !== "POST") {
    return {
      statusCode: 405,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ok: false, code: "unknown", error: "Method not allowed" }),
    };
  }
  ...
  try {
    ...
    if (
      ...
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ok: false, code: "invalid_image", error: "Imagem inválida." }),
      };
    }
    imageDataUrl = body.imageDataUrl;
  } catch {
    return {
      statusCode: 400,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ok: false, code: "invalid_image", error: "Payload inválido." }),
    };
  }

  console.log("V1 DEBUG ENV KEYS:", Object.keys(process.env).filter(k => k.includes("API") || k.includes("KEY")));
  console.log("V1 GEMINI_API_KEY len:", process.env.GEMINI_API_KEY?.length || 0, "starts:", process.env.GEMINI_API_KEY?.substring(0, 6));
  console.log("V1 LOVABLE_API_KEY len:", process.env.LOVABLE_API_KEY?.length || 0, "starts:", process.env.LOVABLE_API_KEY?.substring(0, 6));
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ok: false, code: "unknown", error: `Falha inesperada ao chamar a IA: ${e?.message || e}` }),
    };
  }
};
